# Combination_v2 of path-yolo.py and generate_roi-better_error-padding.py
# Some more error handling and more grace period fixes.
...
def crop_image(image, bbox):
    if bbox is None:
        return image  # Return the original image if no phone
    x1, y1, x2, y2 = map(int, bbox)  # Convert coords to ints (mediapipe gives floats which mess things up)
...
def main():
    ...
    show_pattern = False
    grace_period = 10
    thumb_grace_period = 5
    frames_without_box = 0
    frames_without_thumb = 0
    ...
    while True:
        ...
        frame = resize_image(frame)  # Resize the frame to 480x640

        # YOLO phone detection (class=67)
        ...
        if lm_list:
            thumb_tip = lm_list[4]  # 4 = thumb tip / check for dominant finger is not implemented
        ...
        if frames_without_box < grace_period and last_bbox is not None:
            ...
            draw_list = []  # Reset draw list when showing placeholder
            show_pattern = False

        if not thumb_inside_box:
            frames_without_box += 1

        # Create a composite image
        if show_pattern and frames_without_thumb < thumb_grace_period:
            ...
            if frames_without_thumb >= thumb_grace_period:
