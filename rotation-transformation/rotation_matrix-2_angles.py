# Unstable upgrade from edge_detection-yolo.py
# Getting the angles of the two edges and calculating a combined rotation matrix.
# With some errors that need manual debugging.
# Unreliable z-coordinate by mediapipe
...
def scale_roi(roi, target_shape):
    ...
    if roi_width == 0 or roi_height == 0:
        return create_placeholder(target_shape, "DIVISION BY ZERO, RUN FOR YOUR LIVES")
...
def calculate_rotation_matrix(angle_long, angle_short):
    # Convert angles to rad
    angle_long_rad = np.radians(angle_long)
    angle_short_rad = np.radians(angle_short)

    # Rotation matrix around Z-axis
    Rz = np.array([
        [np.cos(angle_long_rad), -np.sin(angle_long_rad), 0],
        [np.sin(angle_long_rad), np.cos(angle_long_rad), 0],
        [0, 0, 1]
    ])
    ...
    Rx = np.array([
        [1, 0, 0],
        [0, np.cos(angle_short_rad), -np.sin(angle_short_rad)],
        [0, np.sin(angle_short_rad), np.cos(angle_short_rad)]
    ])
            ...
            if lines is not None:
                ...
                if long_edge_line is not None:
                    ...
                    if short_edge_line is not None:
                        ...
                        # Print the angles for each frame
                        print(f"Long edge angle: {angle_long:.2f} degrees")
                        print(f"Short edge angle: {angle_short:.2f} degrees")
                        ...
                        R = calculate_rotation_matrix(angle_long, angle_short)
